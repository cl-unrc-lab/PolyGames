...
import numpy as np
"""
This is a script for generating the Roborta vs Rigoborto example, it has several options:
    -h prints de help
...
def writeBoard(fileName) :  
    ...
    for i in range(length) :
        for j in range(width) :
            if (not plain) :
                qterrain[i][j] = random.uniform(0,0.5)
                mFile.write(f"""// const Q_TERRAIN_{i}_{j} = {qterrain[i][j]};\n""") 
    ...
    for i in range(length) :
        for j in range(width) :
            if (not plain) :
                fslope[i][j] = random.uniform(-1,1)
                mFile.write(f""" //F_SLOPE_{i}_{j} =  {fslope[i][j]};\n""")
    ...
    for i in range(length) :
        for j in range(width) :
            if (not plain) :
                lslope[i][j] = random.uniform(-1,1)
                mFile.write(f"""//L_SLOPE_{i}_{j}   =  {lslope[i][j]};\n""")
    mFile.close();    
...
def writeHeading(fileName) :
    ...
    mFile.write("   roborta, [robl], [robr], [robf], [robb], [rigl_u], [rigr_u], [rigf_u], [rigb_u] \n")
    mFile.write("endplayer\n")
    mFile.write("\n");
    mFile.write("player p2\n")
    mFile.write("  rigoborto, [rigl], [rigr], [rigf], [rigb], [robl_u], [robr_u], [robf_u], [robb_u]\n")
    mFile.write("endplayer\n")
    mFile.write("\n");
    mFile.write(f"const int LENGTH = {length};\n")
    mFile.write("global sched : [0..3] init 0;\n") # 0: roborta selects action, 1: rig selects uncertainty, 2:rig selects action, 3: rob selects uncertainty
    mFile.write(f"""global robcol : [0..{width-1}] init {rob_x}; 
                    global robrow : [0..{length}] init {rob_y};\n""") # roborta position
    mFile.write(f"""global rigcol : [0..{width-1}] init {rig_x};
                    global rigrow : [0..{length}] init {rig_y};
            """)
    mFile.write(f"global robact : [0..3];\n")
    mFile.write(f"global rigact : [0..3];\n")
...
module roborta
    ...
    # roborta selects an action and rigoborto solves the uncertainty
    mFile.write(f"""    [robl] (sched = 0) & !end  -> (sched'=1)&(robact'=0); \n""") # act 0
    mFile.write(f"""    [robr] (sched = 0) & !end  -> (sched'=1)&(robact'=1); \n""") # act 1
    mFile.write(f"""    [robb] (sched = 0) & !end  -> (sched'=1)&(robact'=2); \n""") # act 2
    mFile.write(f"""    [robf] (sched = 0) & !end  -> (sched'=1)&(robact'=3); \n""") # act 3
    mFile.close() 
    # now roborta solves the uncertainty of rigoborto
    rob_uncertainty(fileName)
          

def rob_uncertainty(fileName) :
    """ This function writes the uncertainty rules for roborta, as solved by rigoborto
    """
    global fslope
    global lslope
    global qterrain
    ...
    for i in range(0,length) :
        for j in range(0,width) :
            mFile.write(f"""    [robl_u] (sched = 1) & (robact=0) & (robrow = {i}) & (robcol = {j}) & !end  -U->""")
            mFile.write(f"""
            pl : (robcol' = {max(0, j-1)}) & (sched' = 2) +         // The first four probabilistic options
            pr : (robcol' = {min(width-1, j+1)}) & (sched' = 2) +   // corresponds to environments setbacks
            pf : (robrow' = {i+1}) & (sched' = 2) +
            pb : (robrow' = {max(0, i-1)}) & (sched' = 2) +
            pm : (robcol' = {max(0, j-1)}) & (sched' = 2)            // Command successful
            """)
            mFile.write("{");
            mFile.write(f"""
    ...
    # second action
            if (i < length) :
                mFile.write(f"""    [robr_u] (sched = 1) & (robact = 1) & robrow = {i} & robcol = {j} & !end  -U->""")
                mFile.write(f"""
            pl : (robcol' = {max(0, j-1)}) & (sched' = 2) +         // The first four probabilistic options
            pr : (robcol' = {min(width-1, j+1)}) & (sched' = 2) +   // corresponds to environments setbacks
            pf : (robrow' = {i+1}) & (sched' = 2) +
            pb : (robrow' = {max(0, i-1)}) & (sched' = 2) +
            pm : (robcol' = {min(width-1, j+1)}) & (sched' = 2)     // Command successful
            """)
                mFile.write("{")
                mFile.write(f"""    
            // Success is affected by the quality and inclination of the terrain
            ...
            if (i < length) :
                mFile.write(f"""
            [robf_u] (sched = 1) & (robact=3) &  (robrow = {i}) & (robcol = {j})  & !end  -U->
            pl : (robcol' = {max(0, j-1)}) & (sched' = 2) +         // The first four probabilistic options
            pr : (robcol' = {min(width-1, i+1)}) & (sched' = 2) +   // corresponds to environments setbacks
            pf : (robrow' = {i+1}) & (sched' = 2) +
            pb : (robrow' = {max(0, i-1)}) & (sched' = 2) +
            pm : (robrow' = {i+1}) & (sched' = 2)                   // Command successful 
            """)
                mFile.write("{")
                mFile.write(f"""
            // Success is affected by the quality and inclination of the terrain
            ...
            if (i < length) :
                mFile.write(f"""
            [robb_u] (sched = 1) & (robact=2) & (robrow = {i}) & (robcol = {j})  & !end  -U->
            pl : (robcol' = {max(0, j-1)}) & (sched' = 2) +         // The first four probabilistic options
            pr : (robcol' = {min(width-1, j+1)}) & (sched' = 2) +   // corresponds to environments setbacks
            pf : (robrow' = {i+1}) & (sched' = 2) +
            pb : (robrow' = {max(0, i-1)}) & (sched' = 2) +
            pm : (robrow' = {max(0, i-1)}) & (sched' = 2)           // Command successful
            """)
                mFile.write("{")
                mFile.write(f"""
            // Success is affected by the quality and inclination of the terrain
...
def writeRigoborto(fileName) :
    ...
    mFile.write(f"""    [rigl] (sched = 2) & !end  -> (sched'=3)&(rigact'=0); \n""")
    mFile.write(f"""    [rigr] (sched = 2) & !end  -> (sched'=3)&(rigact'=1); \n""")
    mFile.write(f"""    [rigb] (sched = 2) & !end  -> (sched'=3)&(rigact'=2); \n""")
    mFile.write(f"""    [rigf] (sched = 2) & !end  -> (sched'=3)&(rigact'=3); \n""")
    mFile.close()
    # now roborta solves the uncertainty of rigoborto
    rig_uncertainty(fileName)   

def rig_uncertainty(fileName) :
    global width
    global length
    global rig_x
    global rig_y
    ...
    for i in range(0,length) :
        ...
        # action one
            mFile.write(f"""
                [rigl_u] (sched = 3) & (rigact=0) & (rigrow = {i}) & (rigcol = {j}) & !end  -U->
            ...
            #action two
            mFile.write(f""" 
                [rigr_u] (sched = 3)& (rigact=1) & (rigrow = {i}) & (rigcol = {j}) & !end  -U->
                pl : (rigcol' = {max(0, j-1)}) & (sched' = 0) +         // The first four probabilistic options
                pr : (rigcol' = {min(width-1, j+1)}) & (sched' = 0) +   // corresponds to environments setbacks
                pf : (rigrow' = {min(length-1, i+1)}) & (sched' = 0) +
                pb : (rigrow' = {max(0, i-1)}) & (sched' = 0) +
            ...
            #action three
            mFile.write(f"""
                [rigf_u] (sched = 3)& (rigact=3) & !end  & rigrow = {i} & rigcol = {j} -U->
                pl : (rigcol' = {max(0, j-1)}) & (sched' = 0) +         // The first four probabilistic options
                pr : (rigcol' = {min(width-1, j+1)}) & (sched' = 0) +   // corresponds to environments setbacks
                pf : (rigrow' = {min(length-1, i+1)}) & (sched' = 0) +
                pb : (rigrow' = {max(0, i-1)}) & (sched' = 0) +
            ...
            #action fourth
            mFile.write(f"""
                [rigb_u] (sched = 3) & (rigact=2) &  rigrow = {i} & rigcol = {j} & !end  -U->
                pl : (rigcol' = {(max(0, j-1))}) & (sched' = 0) +         // The first four probabilistic options
                pr : (rigcol' = {(min(width-1, j+1))}) & (sched' = 0) +   // corresponds to environments setbacks
                pf : (rigrow' = {(min(length-1, i+1))}) & (sched' = 0) +
                pb : (rigrow' = {max(0, i-1)}) & (sched' = 0) +
