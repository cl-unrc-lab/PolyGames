...
public class CommandWithArraysReplacer extends ASTElementReplacer {
	...
	@Override
	/**
	 * This replaces a command where there is an occurrence of an array
	 */
	public ASTElement replace(ASTElement astElement, ExpressionArray expressionArrayIndex, Expression expression, int index)
		...
		// the following set will be used to recursively replace
		setExpression(expression); 
		setExpressionArrayIndex(expressionArrayIndex);
		// a deep clone of the command
		Command command = (Command) astElement.clone().deepCopy(new DeepCopy()); // astElement.accept(new DeepCopy());

		// each guard is overwritten 
		// it says that the  index has to be the same as the expression array index
		command.setGuard(
	...
	@Override
	/**
	 * Visits the updates replacing ExpressionArrayIndex by expression
	 */
